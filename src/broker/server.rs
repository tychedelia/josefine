...
impl Server {
    ...
    pub async fn run(self, client: RaftClient, broker: Broker) -> Result<()> {
        ...
        let (task, tcp_receiver) =
            tcp::receive_task(crate::logger::get_root_logger().new(o!()), listener, in_tx)
                .remote_handle();
        tokio::spawn(task);

        let (task, handle_messages) =
            handle_messages(log.new(o!()), client, broker, out_tx).remote_handle();
        tokio::spawn(task);
