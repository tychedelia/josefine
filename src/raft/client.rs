...
pub struct RaftClient {
    request_tx: UnboundedSender<(Proposal, oneshot::Sender<std::result::Result<Response, ResponseError>>)>,
}

impl RaftClient {
    /// Creates a new Raft client.
    pub fn new(request_tx: UnboundedSender<(Proposal, oneshot::Sender<std::result::Result<Response, ResponseError>>)>) -> Self {
        Self { request_tx }
    }
    ...
    async fn request(&self, request: Proposal) -> Result<Response> {
        ...
        response_rx.await?.map_err(|e| anyhow::anyhow!("error executing request {}", e))
    }
