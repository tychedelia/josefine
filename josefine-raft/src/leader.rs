...
impl Raft<Leader> {
    ...
    fn append(&mut self, data: Vec<u8>) -> Result<LogIndex> {
        ...
        for node in &self.config.nodes {
            let progress = self.role.progress.get(node.id);
            if let Some(NodeProgress::Replicate(progress)) = progress {
                let term = self.state.current_term;
                let prev = self.log.get(progress.index)?.expect("previous entry does not exist!");
                let entries = self.log.get_from(progress.next)?;
